...
Page({
  ...
  // 监听编辑弹窗打开
  onEditPopupOpened() {
    this.setData({ hasEditPopupOpen: true });
  },

  // 监听编辑弹窗关闭
  onEditPopupClosed() {
    this.setData({ hasEditPopupOpen: false });
  },
  ...
  async getCollectionList() {
    ...
    try {
      ...
      this.setData({ collectionList: sortedList }, () => {
        console.log('收藏夹数量：', this.data.collectionList.length);
      });
    } catch (err) {
      wx.showToast({ title: err || '加载失败', icon: 'error' });
  ...
  async saveCreateCollection() {
    const { collectionList, tempName, tempDesc } = this.data;

    // 校验：名称不能为空
    if (!tempName.trim()) {
      wx.showToast({ title: '收藏夹名称不能为空', icon: 'none' });
      return;
    }
    // 校验：名称长度
    if (tempName.length > MAX_NAME_LENGTH) {
      wx.showToast({ title: `名称不能超过${MAX_NAME_LENGTH}个字`, icon: 'none' });
      return;
    }
    ...
    try {
      ...
      wx.showToast({ title: '收藏夹创建成功', icon: 'success' });
      // 关闭浮窗+清空临时数据
      this.closeCreatePopup();
      // 刷新列表
      this.getCollectionList();
    } catch (err) {
      wx.showToast({ title: err || '创建失败', icon: 'error' });
