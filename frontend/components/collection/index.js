...
Component({
    ...
    methods: {
      ...
      navigateToCollectionDetail() {
        ...
        if (!collectionId) {
          wx.showToast({ title: '收藏夹ID无效', icon: 'none' });
          return;
        }
      ...
      async saveCollectionEdit() {
        const collectionId = String(this.properties.item.id || '').trim();
        const { tempName, tempDesc } = this.data;
        ...
        if (tempDesc.length > MAX_DESC_LENGTH) {
          wx.showToast({ title: `描述不能超过${MAX_DESC_LENGTH}个字`, icon: 'none' });
          return;
        }
        ...
        try {
          ...
          this.triggerEvent('editPopupClosed');
          // 刷新列表
          this.triggerEvent('collectionUpdated', { id: collectionId, data: updatedData });
        } catch (err) {
          ...
          success: async (res) => {
            if (res.confirm) {
              try {
                ...
                this.triggerEvent('editPopupClosed');
                // 触发事件通知父组件刷新收藏夹列表
                this.triggerEvent('collectionDeleted', { id: collectionId });
              } catch (err) {
                wx.hideLoading();
