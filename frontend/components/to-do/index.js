...
Component({
  ...
  attached() {
    // 标记timeRange已初始化
    this.setData({
      timeRangeInited: false
    });
    // 初始化时间选择器选项
    this.initTimeRange();
    // 初始化完成后标记状态
    this.setData({
      timeRangeInited: true
    });
    // 执行暂存的时间初始化逻辑
  ...
  methods: {
    ...
    resetModal() {
      this.setData({
        ...
        isEdit: false,
        tempData: {
          ...this.properties.todoData
        }
      });
    },
    handleTitleInput(e) {
      this.setData({
        'tempData.title': e.detail.value
      });
    },
    handleContentInput(e) {
      this.setData({
        'tempData.content': e.detail.value
      });
    }
  }
});